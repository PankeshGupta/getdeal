...
def unseen_notices(request):
    """
    The unseen notices view. displayed on the header of the website.
    
    Template: :template:`notification/unseen_notices.html`
    ...
    return render_to_response("notifications/unseen_notices.html", {
        "notices": notices,
...
def notices(request):
    ...
    return render_to_response("notifications/notices.html", {
        "notices": notices,
    }, context_instance=RequestContext(request))
...
def notice_settings(request):
    ...
    return render_to_response("notifications/notice_settings.html", {
...
def single(request, id, mark_seen=True):
    ...
    if request.user == notice.recipient:
        if mark_seen and notice.unseen:
            notice.unseen = False
            notice.save()
        return render_to_response("notifications/single.html", {
            "notice": notice,
        }, context_instance=RequestContext(request))
    raise Http404
