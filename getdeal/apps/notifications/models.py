...
NOTICE_MEDIA = (
    ("1", _("Email")),
    #@change: add a new media (onsite) to allow sonsite setting by mourafiq
    ("0", _("Onsite")),
...
def get_notification_setting(user, notice_type, medium):
    try:
        ...
        #@change: add a new media (onsite) to allow onsite setting
        if medium == "0":
            default = True
        else:
...
#@change: add a new media (onsite) to allow sonsite setting
def allow_onsite(user, notice_type, medium):
    return get_notification_setting(user, notice_type, medium).send
...
def send_now(users, label, extra_context=None, on_site=True, sender=None):
    ...
    for user in users:
        ...
        notice = Notice.objects.create(recipient=user, message=messages['notice.html'],
                                       notice_type=notice_type, on_site=allow_onsite(user, notice_type, "0"),
                                       sender=sender)
        #@change: end of changes
